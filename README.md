...
| 40 | [EC2 인스턴스 타입은 무엇을 기준으로 고르는가](daily/day40-ec2-sizing.md) | 고르는 건 "얼마나 빠른가"가 아니라 "얼마나 오래 빠를 수 있는가". 작은 인스턴스는 CPU·네트워크·EBS 세 개의 크레딧 통을 따로 태운다 |
| 41 | [커버링 인덱스 — 인덱스를 타고도 남는 비용을 없애는 법](daily/day41-covering-index.md) | 인덱스 탐색은 일의 절반이고 나머지 절반은 행을 가지러 가는 랜덤 접근. 커버링은 인덱스의 속성이 아니라 인덱스와 쿼리의 관계라 `SELECT` 컬럼 하나에 깨진다 |
